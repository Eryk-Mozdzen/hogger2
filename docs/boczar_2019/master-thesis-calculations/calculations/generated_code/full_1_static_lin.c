#include "full_1_static_lin.h"


#include "robot_parameters.h"

void full_1_feedback_u(float u[5], const float K[25], const float q[9], const float h[5], const float hd[5], const float d_hd[5]) {
...
void full_1_static_lin_eta(float eta[5], const float u[5], const float q[9]) {
    ...
    eta[2] = u[3];
    eta[3] = -u[4]*sinf(q[7]) + u[0]*sinf(q[2])/R - u[1]*cosf(q[2])/R;
    eta[4] = u[4]*cosf(q[7])*tanf(q[6]) + 2*l*u[2]/(R*cosf(q[6])) + u[0]*cosf(q[2])/(R*cosf(q[6])) + u[1]*sinf(q[2])/(R*cosf(q[6]));
}
float full_1_static_lin_det_D(const float q[]) {
