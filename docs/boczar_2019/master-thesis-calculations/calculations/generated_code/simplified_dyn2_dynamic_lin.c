#include "simplified_dyn2_dynamic_lin.h"


#include "robot_parameters.h"

void simplified_dyn2_feedback_v(float v[5], const float K1[25], const float K2[25], const float q[9], const float h[5], const float d_h[5], const float hd[5], const float d_hd[5], const float d2_hd[5]) {
...
void simplified_dyn2_dynamic_lin_u(float u[5], const float v[5], const float eta[5], const float q[9]) {
    ...
    u[0] = -(v[0] + (1.0F/2.0F)*powf(eta[1], 2)*powf(q[7], 2)*sinf(q[2] + q[3])*sinf(q[3] - q[5])/(l*sinf(q[5])))*sinf(q[2] + q[3])/(eta[1]*q[7]) + (v[1] - 1.0F/2.0F*powf(eta[1], 2)*powf(q[7], 2)*sinf(q[3] - q[5])*cosf(q[2] + q[3])/(l*sinf(q[5])))*cosf(q[2] + q[3])/(eta[1]*q[7]);
    u[1] = v[3];
    u[2] = v[2];
    u[3] = -v[3]*q[7]/eta[1] + (v[0] + (1.0F/2.0F)*powf(eta[1], 2)*powf(q[7], 2)*sinf(q[2] + q[3])*sinf(q[3] - q[5])/(l*sinf(q[5])))*cosf(q[2] + q[3])/eta[1] + (v[1] - 1.0F/2.0F*powf(eta[1], 2)*powf(q[7], 2)*sinf(q[3] - q[5])*cosf(q[2] + q[3])/(l*sinf(q[5])))*sinf(q[2] + q[3])/eta[1];
    u[4] = -(v[0] + (1.0F/2.0F)*powf(eta[1], 2)*powf(q[7], 2)*sinf(q[2] + q[3])*sinf(q[3] - q[5])/(l*sinf(q[5])))*q[8]*sinf(q[2])/(eta[1]*q[7]*sinf(q[3])) + (v[1] - 1.0F/2.0F*powf(eta[1], 2)*powf(q[7], 2)*sinf(q[3] - q[5])*cosf(q[2] + q[3])/(l*sinf(q[5])))*q[8]*cosf(q[2])/(eta[1]*q[7]*sinf(q[3])) - (v[4] + eta[1]*eta[2]*q[7]*sinf(q[3])*cosf(q[5])/(q[8]*powf(sinf(q[5]), 2)))*powf(q[8], 2)*sinf(q[5])/(eta[1]*q[7]*sinf(q[3]));
}
float simplified_dyn2_dynamic_lin_det_Kdd(const float q[], const float eta[]) {
