...
void HAL_GPIO_EXTI_Rising_Callback(uint16_t GPIO_Pin) {
    HAL_I2C_Mem_Read_DMA(imu.i2c, MPU6050_ADDR << 1, MPU6050_REG_ACCEL_XOUT_H, 1, imu.buffer,
                         sizeof(imu.buffer));
}
...
static void init(imu_t *imu) {
    HAL_I2C_RegisterCallback(&hi2c1, HAL_I2C_MEM_RX_COMPLETE_CB_ID, isr_memory_received);

    mpu6050_write(imu, MPU6050_REG_PWR_MGMT_1, MPU6050_PWR_MGMT_1_DEVICE_RESET);
    ...
    mpu6050_write(imu, MPU6050_REG_ACCEL_CONFIG, MPU6050_ACCEL_CONFIG_RANGE_4G);

    mpu6050_write(imu, MPU6050_REG_GYRO_CONFIG, MPU6050_GYRO_CONFIG_RANGE_500DPS);

    mpu6050_write(imu, MPU6050_REG_SMPLRT_DIV, 0);
}

static void loop(imu_t *imu) {
    if(imu->ready) {
        imu->ready = 0;
...
static void serialize(cmp_ctx_t *cmp, void *context) {
    ...
    cmp_write_float(cmp, imu.acceleration[0]);
    cmp_write_float(cmp, imu.acceleration[1]);
    cmp_write_float(cmp, imu.acceleration[2]);
    cmp_write_str(cmp, "gyroscope", 9);
    cmp_write_array(cmp, 3);
    cmp_write_float(cmp, imu.gyration[0]);
    cmp_write_float(cmp, imu.gyration[1]);
    cmp_write_float(cmp, imu.gyration[2]);
}
