...
Window::Window(QWidget *parent) : QWidget(parent) {
...

    {
        ...
        connect(timer, &QTimer::timeout, [this]() {
            const double v = gamepad.get(Gamepad::Analog::LY);
            const double w = 10*gamepad.get(Gamepad::Analog::LX);
            ...
            pwm[2] = gamepad.get(Gamepad::Button::A) ? 1100 : 1000;

            pwm[3] = sliders[2]->value() + lerp(-a2, -M_PI, M_PI, 1000, 2000);
            pwm[4] = sliders[3]->value() + 1500;
            pwm[5] = gamepad.get(Gamepad::Button::A) ? 1100 : 1000;
